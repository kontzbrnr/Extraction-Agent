...
class DeepSearch(RAGAgent):
    ...
    def _get_current_collections(self) -> List[str]:
        """Return the current vector DB collections for this embedding dimension.

        This avoids relying on the collection snapshot captured when the agent
        was instantiated, which can become stale after runtime load operations.
        """
        collection_infos = self.vector_db.list_collections(dim=self.embedding_model.dimension)
        current_collections = [
            collection_info.collection_name for collection_info in collection_infos
        ]
        if current_collections:
            self.collection_router.all_collections = current_collections
            return current_collections
        return self.collection_router.all_collections
    ...
    async def _search_chunks_from_vectordb(self, query: str, sub_queries: List[str]):
        ...
        if self.route_collection:
            ...
            selected_collections = self._get_current_collections()
            n_token_route = 0
    ...
    def query_raw(self, original_query: str, max_iter: int = 3) -> List[dict]:
        """Return reconstructed article text from vector DB chunks without LLM usage.

        This method bypasses all LLM-based query decomposition, routing, reranking,
        reflection, and summarization. It performs direct vector retrieval using the
        current embedding model and vector DB, then groups chunk texts by reference.

        Args:
            original_query: User query text.
            max_iter: Reserved for API compatibility. Not used in raw mode.

        Returns:
            A list of dicts:
                [{"title": str, "url": str, "rawText": str}, ...]
        """
        del max_iter
        ...
        for result in retrieved_results:
            reference_url = result.reference or ""
            if not reference_url:
                continue

            chunk_text = result.text or ""
            if reference_url not in articles:
                articles[reference_url] = []

            articles[reference_url].append(chunk_text)

            title_value = ""
            if isinstance(result.metadata, dict):
                title_value = str(result.metadata.get("title", "") or "")
            if reference_url not in article_titles and title_value:
                article_titles[reference_url] = title_value

        reconstructed = []
        for reference_url in sorted(articles.keys()):
            article_text = " ".join(articles[reference_url])
            article_text = re.sub(r"\s+", " ", article_text).strip()
            reconstructed.append(
                {
                    "title": article_titles.get(reference_url, ""),
                    "url": reference_url,
