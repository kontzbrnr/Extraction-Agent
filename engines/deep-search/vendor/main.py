...
import deepsearcher.configuration as runtime_configuration
...
def query_raw(
    ...
    max_iter: int = Query(
        3,
        description="Reserved for compatibility; raw retrieval does not iterate via LLM.",
        ge=1,
        examples=[3],
    ),
):
    """Return reconstructed article text from vector DB chunks without LLM reasoning."""
    try:
        default_searcher = runtime_configuration.default_searcher
        deepsearch_instance = next(
            (agent for agent in default_searcher.rag_agents if hasattr(agent, "query_raw")),
            None,
        )
        if deepsearch_instance is None:
            raise RuntimeError("DeepSearch instance with query_raw not available")

        results = deepsearch_instance.query_raw(original_query, max_iter=max_iter)
